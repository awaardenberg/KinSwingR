...
#' Default="|"
#' @param multi.protein.delimiter The character used to delimit multi-protein 
#' assignments. Default=":". E.g. Ddx17:Ddx2
#' @param multi.site.delimiter The character used to delimit multi-site 
#' assignments. Default=";". E.g. 494;492
#' @param seq.number The annotation frame that contains the sequence after 
#' delimitation. E.g. The sequence "RSRYRTTSSANNPN" is contained in the 4th 
#' annotation frame of the following annotation: 
#' "A0A096MIX2|Ddx17|494|RSRYRTTSSANNPN" and would therefore set seq.number=4.
#' Default=4
...
#'  combination with replace.search and replace.with to replace amino acids. 
#'  Options are "TRUE" or "FALSE". Default="FALSE".
#' @param replace.search Amino Acid to search for when replacing sequences. 
#' Default="X"
#' @param replace.with Amino Acid to replace with when replacing sequences. 
#' Default="_"
...
cleanAnnotation <- function(input.data = NULL,
  ...
  seq.list <-
    strsplit(data.annotation, annotation.delimiter, fixed = TRUE)
  #get the nth element (seq.number) from the list (that contains the sequences)
  seqs <- sapply(seq.list, `[`, seq.number)
  #2. multi-site assignment
  seq.list <-
    strsplit(as.character(seqs), multi.site.delimiter, fixed = TRUE)
  seqs <- unlist(seq.list)
  #3. multi-gene assignment
  seq.list <-
    strsplit(as.character(seqs), multi.protein.delimiter, fixed = TRUE)
  seqs <- unlist(seq.list)
  ...
  data.out <-
    data.frame(
      "annotation" = data.out$annotation,
      "peptide" = data.out$seqs,
      "fc" = data.out$fc,
      "pval" = data.out$pval
    )
...
# @param seq.length Full length of substrate sequence (default is 15). Will be 
# trimmed automatically or report error if sequences in kinase.table are not 
# long enough.
# @param verbose Print progress to screen. Default = FALSE
#
