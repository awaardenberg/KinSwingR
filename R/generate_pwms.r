...
#'
#' @description Generate Position Weight Matrices (PWMs) for a table containing centered
...
#' @param kinase.table A data.frame of substrate sequences and kinase names. Format of data must
#' be as follows: column 1 - kinase/kinase family name/GeneID, column 2 - centered peptide
#' seqeuence.
#' @param wild.card Letter to describe sequences that are outside of the protein after centering
#' on the phosphosite (e.g ___MERSTRELCLNF). Default: "_".
#' @param substrate.length Full length of substrate sequence (default is 15). Will be trimmed
#'  automatically or report error if sequences in kinase.table are not long enough.
#' @param substrates.n Number of sequences used to build a PWM model. Low sequence counts will
#' produce poor representative PWM models. Default: "10"
#' @param pseudo Small number to add to values for PWM log transformation to prevent log
#' transformation of zero. Default = 0.01
#' @param remove.center Remove all peptide seqeuences with the central amino acid matching a
...
#'
#' @return Output is a list containing two tables, "pwm" and "kinase". To access PWMs:
...
build.pwm <- function(kinase.table = NULL,
    ...
    kinase.table[, 2] <- trim.seqs(kinase.table[, 2], seq.length=substrate.length, verbose=verbose)
    ...
    if (remove.center != FALSE) {
      ...
      center.aa <- substr(kinase.table[, 2], half.window, half.window)
      if(verbose){
        print(paste(
          "You have selected to remove", length(which(center.aa == toupper(remove.center))),
          "peptide sequences for building PWMs that contain a centered letter of:",
          toupper(remove.center), sep = " "))
      }
      kinase.table <- kinase.table[center.aa != toupper(remove.center), ]
    }
    ...
    if(verbose){
      cat(paste(nrow(kinase.table), "unique kinase:substrate sequences in table provided.",
                  sep=" "))
    }
    ...
    kinase.summary$n <-
      c(sapply(1:nrow(kinase.summary), function(i)
        length(kinase.table[, 1][kinase.table[, 1] == kinase.summary[i, 1]])))

    #filter the list here which is used to build pwms:
    kinase.summary <- kinase.summary[kinase.summary$n >= substrates.n,]

    #generate PWM list:
    pwm.list <-
      c(lapply(1:nrow(kinase.summary), function(i)
        score.pwm(as.character(kinase.table[, 2][kinase.table[, 1] == kinase.summary[i, 1]]),
                  substrate.length=substrate.length, wild.card=wild.card, pseudo=pseudo)))

    return(list("pwm" = pwm.list, "kinase" = kinase.summary))
  }

# describeIn build.pwm this helper function performs the calculations for building the PWMs
...
score.pwm <-
  function(input.data,
           ...
           wild.card = "_",
           pseudo = 0.01) {
    #reformat input.data; build a matrix of the sequences (split into individual AA's) = substrate.length
    input.data <-
      data.frame(matrix(unlist(strsplit(input.data , "")) , ncol = substrate.length , byrow = TRUE))
    uniq.AA <-
      c(wild.card, "A", "C", "D", "E", "F", "G", "H", "I", "K", "L", "M", "N", "P", "Q",
        "R", "S", "T", "V", "W", "Y")
    #1. Generate PFM:
    pwm <-
      t(cbind(sapply(1:length(uniq.AA), function(i)
        (
          sapply(1:ncol(input.data), function(j)
            sum(
              ifelse(as.character(input.data[, j]) == as.character(uniq.AA[i]), 1, 0)
            ))
        ))))
    rownames(pwm) <- uniq.AA
    colnames(pwm) <- paste("p", 1:ncol(pwm), sep = "")
    #remove "_"
    #SET AS AN OPTION:
