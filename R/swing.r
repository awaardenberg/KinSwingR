...
#' is a normalised and weighted score of predicted kinase activity. If permutations
#' are selected, network node:edges are permutated. P-values will be calculated for
#' both ends of the distribution of swing scores (positive and negative swing scores).
...
#'  This must be the same dataframe used in score.sequences()
...
#' @param permutations Number of permutations to perform. This will shuffle the kinase-subtrate
#' edged of the network n times. To not perform permutations and only generate the scores,
#' set permutations=1 or permutations=FALSE. Default: "1000"
#' @param seed This is for reproducible results for permutation testing.
#' To not use a set seed=NULL. Default: "1234"
  ...
  function(input.data = NULL,
    ...
    if (p.cut.fc >= 1)
      stop("p.cut.fc needs to be less than 1; make sure your p-values are not
           log transformed")
    if (permutations != FALSE & (!is.numeric(permutations) | permutations < 1))
      stop("permutations needs to be a numerical number. Either 1 or FALSE (for no permutation) or greater than 1 to set
           the number of pemutations")
    ...
    input.data$annotation <- paste(input.data$annotation, input.data$peptide, sep="::")
    pwm.pval$annotation <- paste(pwm.pval$annotation, pwm.pval$peptide, sep="::")
    data.merge <- unique(merge(input.data, pwm.pval, by = "annotation"))

    #3. Swing scores of REAL data.
    swing.out <- swing.score(data.merge=data.merge,
                             pwm.in=pwm.in,
                             permute=FALSE,
    ...
    if(permutations != FALSE && permutations > 1){
        ...
        set.seed(seed)
        n.permute <- lapply(1:permutations, function(i)
                                  sample(as.character(colnames(data.merge))[7:ncol(data.merge)],
                                  length(colnames(data.merge[,7:ncol(data.merge)])),
                                  replace=FALSE))
        # not really required
        names(n.permute) <- paste("rand", c(1:permutations), sep="")
        # calculate swing scores and return in a single table
        swing.permute <- list(bplapply(1:permutations, function(i)
                                      swing.score(data.merge = setNames(data.frame(data.merge),
        ...
        swing.permute <- data.frame(Reduce(merge, swing.permute[[1]]))

        if(verbose){
          cat("[Step3/3] : Calculating p-values\n")
        }
        #obtain p-values two sided, test independently...
        swing.out$p.greater <- unlist(bplapply(1:nrow(swing.out), function(i)
                                      (sum(ifelse(as.numeric(swing.permute[swing.permute$kinase ==
                                      swing.out$kinase[i],][2:ncol(swing.permute)]) >
                                      as.numeric(swing.out$swing.raw[i]), 1, 0), na.rm = TRUE)+1)
                                      /(as.numeric(permutations)+1),
                                      BPPARAM = MulticoreParam(workers=threads)))

        swing.out$p.less <- unlist(bplapply(1:nrow(swing.out), function(i)
                                      (sum(ifelse(as.numeric(swing.permute[swing.permute$kinase ==
                                      swing.out$kinase[i],][2:ncol(swing.permute)]) <
                                      as.numeric(swing.out$swing.raw[i]), 1, 0), na.rm = TRUE)+1)
                                      /(as.numeric(permutations)+1),
        ...
        #correct p-values to NaN where all counts are zero (i.e. no p-value is computable)
    ...
    network <- data.table::melt(network, id.vars = c("annotation"),
                                measure.vars = colnames(network[7:ncol(network)]))
    # keep "significant" edges
    network <- network[network$value == 1,]
    network <- data.frame("source"=as.character(network$variable),
...
swing.score <- function(data.merge, pwm.in, pseudo.count = 1, permute=FALSE, n=1) {
  ...
  p.n.all$swing.raw <- log2(p.n.all$swing.raw) * log2(p.n.all$n + pseudo.count)
  #weighted by size of kinase-substrate network:
  p.n.all$swing.raw <- p.n.all$swing.raw * log2(p.n.all$all + pseudo.count)
  #z-score transform
  p.n.all$swing <-
